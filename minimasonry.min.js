var MiniMasonry = function () {
    ...
    return t.prototype.init = function (t) {
        ...
        if (this._currentGutterX = this.conf.gutterX, this._currentGutterY = this.conf.gutterY, this._container = "object" == typeof this.conf.container && this.conf.container.nodeName ? this.conf.container : document.querySelector(this.conf.container), !this._container)
            throw new Error("Container not found or missing");
        var e = this.resizeThrottler.bind(this);
        window.addEventListener("resize", e),
            this._removeListener = function () { window.removeEventListener("resize", e) },
            this.layout()
        ...
        this._width < t && (this._width = t, this._container.style.minWidth = t + "px"),
            1 == this.getCount() ? (this._currentGutterX = this.conf.ultimateGutterX, this._currentGutterY = this.conf.ultimateGutterY, this._count = 1) : this._width < this.conf.baseWidth + 2 * this._currentGutterX ? this._currentGutterX = 0 : this._currentGutterX = this.conf.gutterX
        ...
        if (this._container) {
            ...
            e = "ltr" == this.conf.direction ? this.conf.surroundingGutter ? this._currentGutterX : 0 : this._width - (this.conf.surroundingGutter ? this._currentGutterX : 0),
                this._count > this._sizes.length && (n = this._sizes.length * (t + this._currentGutterX) - this._currentGutterX, !1 === this.conf.wedge ? e = "ltr" == this.conf.direction ? (this._width - n) / 2 : this._width - (this._width - n) / 2 : "ltr" == this.conf.direction || (e = this._width - this._currentGutterX));
            for (var o = 0; o < r.length; o++) {
                ...
                r[o].style.transform = "translate3d(" + Math.round(c) + "px," + Math.round(u) + "px,0)",
                    this._columns[h] += this._sizes[o] + (1 < this._count ? this._currentGutterY : this.conf.ultimateGutterY)
            }
